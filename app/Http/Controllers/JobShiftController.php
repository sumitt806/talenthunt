...
class JobShiftController extends AppBaseController
{
    ...
    {
        if ($request->ajax()) {
            return Datatables::of((new JobShiftDataTable())->get())->make(true);
        }
     ...
     * @return JsonResource
     */
    public function store(CreateJobShiftRequest $request)
    {
        $input = $request->all();
        $this->jobShiftRepository->create($input);

        return $this->sendSuccess('Job Shift saved successfully.');
    }
