...
class FunctionalAreaController extends AppBaseController
{
     ...
     * @return JsonResource
     */
    public function store(CreateFunctionalAreaRequest $request)
    {
        $input = $request->all();
        $this->functionalAreaRepository->create($input);

        return $this->sendSuccess('Functional Area saved successfully.');
    }
