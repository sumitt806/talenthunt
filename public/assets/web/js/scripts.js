        ...
        if(active==true) {
          ...
          me.parent().find('> .dropdown-menu').slideUp(500, function() {            
            ...
            me.parent().find('> .dropdown-menu').slideDown(500, function () {
                if (typeof currentUrlName !== 'undefined' || currentUrlName !=
                    null) {
                    me.parent().
                        find('> .dropdown-menu').
                        find('a').
                        each(function () {
                            let drpHref = $(this).attr('href');
                            let drpMainMenuUrl = currentUrlName.substr(0,
                                drpHref.length);
                            if (drpHref === drpMainMenuUrl)
...
$(document).on('click', '.nav-collapse-toggle', function (event) {
    ...
    if(w.outerWidth() <= 1024) {
     $('.searchTerm').slideDown(300);
      ...
      update_sidebar_nicescroll();
        ...
        $('.searchTerm').slideUp(300);
        body.removeClass('search-show search-gone');
      if(body.hasClass('sidebar-mini')) {
          $('.searchTerm').show();
          if ($(document).find('.side-menus').hasClass('active')) {
              $(document).
                  find('.side-menus.active > ul').
                  css('display', 'block');
          }
  ...
  var toggleLayout = function() {
    ...
    if(w.outerWidth() <= 1024) {
      ...
      if(now_layout_class == 'layout-3') {
        ...
        main_sidebar.find('.navbar-nav').
            addClass('sidebar-menu').
            removeClass('navbar-nav');
          main_sidebar.find('.sidebar-menu .nav-item.dropdown.show a').
              trigger('click');
    ...
    $(document).on('click', ".search-sign", function (event) {
        let w = $(window), body = $('body');
        if (body.hasClass('sidebar-mini')) {
            $('.searchTerm').slideDown(300);
            toggle_sidebar_mini(false);
        }
